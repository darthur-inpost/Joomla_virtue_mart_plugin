 ...
 * @copyright Copyright (c) 2004 - 2015 InPost UK Ltd. All rights reserved.
...
class VirtuemartViewInpostparcels extends VmViewAdmin
{
	function display($tpl = null)
	{
		...
		if($curTask == 'edit')
		{
			...
			// Prevent the save option once the parel has had it's
			// status changed.
			if ($parcel->parcel_status == 'Created' ||
				$parcel->parcel_status == '0')
			{
		...
		else
		{
			...
			$orderStatusModel =VmModel::getModel('inpostparcels');
	...
	public function renderParcelstatesList()
	{
		...
		return JHtml::_( 'select.genericlist', inpostparcels_helper::getParcelStatus(),  $parcelstates, 'parcel_status', "", 'class="inputbox" onchange="this.form.submit();"');
	}
